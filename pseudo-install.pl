...
Usage: pseudo-install.pl -[G|C][feA][l|c|u] [all|default|developer|<module>
  ...
  -[A]utoconf  - create simple LocalSite.cfg from Foswiki & Config.spec defaults
...
sub init {
    ...
    if ($CAN_LINK) {
        $install    = 'link';
        $symlink_fn = \&symlink_perl;
    }
    else {
        local $ENV{PATH} = untaint( $ENV{PATH} );
        if (
            !system( 'cmd /c mklink "testlink' . $n . '" "testgt' . $n . '"' ) )
        {
            $CAN_LINK   = 1;
            $install    = 'link';
            $symlink_fn = \&symlink_windows_mklink;
        }
        else {
            $install = 'copy';
        }
...
sub install {
    ...
    if ( $install eq 'link' ) {
        just_link( $moduleDir, $dir, $file, $ignoreBlock );
    }
    elsif ( $install eq 'copy' ) {
        copy_in( $moduleDir, $dir, $file, $ignoreBlock );
    }
    elsif ( !$install ) {
        uninstall( $moduleDir, $dir, $file, $ignoreBlock );
    }
    else {
        die "Don't know how to '" . ( $install || 'undef' ) . "'\n";
    }
        ...
        && -f File::Spec->catfile( $moduleDir, $1 . $3 ) )
    {
        ...
        for my $kind (qw( .uncompressed .compressed _src )) {
            ...
            if ( -f File::Spec->catfile( $moduleDir, $srcfile ) ) {
                linkOrCopy($moduleDir, $srcfile, $file);
                $found++;
                last;
            }
        }
...
sub _cleanPath {
    ...
    if ( !Files::Spec->file_name_is_absolute($path)) {
        $path = File::Spec->catfile( $base, $path ) if $base;
        $path = File::Spec->rel2abs( $path, $basedir );
    }
    $path = File::Spec->canonpath($path);

    # PH commented the following:
    # while ( $path =~ s#/[^/]+/\.\.## ) { }

    # File::Spec::canonpath: "If /foo on your system is a symlink to /bar/baz,
    # then /foo/../quux is actually /bar/quux, not /quux as a naive ../-removal
...
sub just_link {
    ...
    foreach my $c (@components) {
        if ( -l File::Spec->catfile( @path_parts, $c ) ) {
            _checkLink( $moduleDir, File::Spec->catfile(@path_parts), $c )
              unless $ignoreBlock;
            last;
        }
        elsif ( -d File::Spec->catdir( @path_parts, $c ) ) {
            push( @path_parts, $c );
        }
        elsif ( -e File::Spec->catfile( @path_parts, $c ) ) {
            error( File::Spec->catfile( @path_parts, $c ) . " is in the way\n" )
              unless $ignoreBlock;
            last;
        }
        elsif (( $c eq 'TWiki' )
            || ( $c eq 'Plugins' && $path_parts[-1] =~ m#^(Fosw|TW)iki$# ) )
        {    # Special case
            my $relpath = File::Spec->catfile( @path_parts, $c );
            my $abspath;

            push( @path_parts, $c );
            $abspath = _cleanPath( File::Spec->catfile(@path_parts) );
            print "mkdir $abspath\n";
            if ( !mkdir($abspath) ) {
            ...
            my $tgt =
              _cleanPath( File::Spec->catfile( $base, @path_parts, $c ) );
            if ( -e $tgt ) {
                my $relpath = File::Spec->catfile( @path_parts, $c );
                my $linkpath =
                  _cleanPath( File::Spec->catfile( @path_parts, $c ) );

                die "Failed to link $linkpath to $tgt: $!"
                  unless &{$symlink_fn}( $tgt, $linkpath );
...
sub symlink_windows_mklink {
    my ( $src, $dst ) = @_;
    local $ENV{PATH} = untaint( $ENV{PATH} );
    return !system("cmd /c mklink \"$dst\" \"$src\"");
}
...
sub run {
    ...
    for my $arg (@ARGV) {
        ...
        @modules = grep { $_ ne 'core' } @modules unless $install;
    }
    ...
    foreach my $module (@modules) {
        ...
        if ($libDir) {
            ...
            if ( ( !$install || $autoenable ) && $module =~ /Plugin$/ ) {
                enablePlugin( $module, $install, $libDir );
            }
        }
    }
    ...
    foreach my $old_rule ( @{$old_rules} ) {
        ...
        # The line is a rule
            ...
            # The line is a path/filename
                ...
                if ( $install || ( !exists $input_files->{$old_rule} ) ) {
                    push( @merged_rules, $old_rule );
                }
                else {
